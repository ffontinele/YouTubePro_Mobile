            ...
            try {
                ...
                if (url == null) {
                    ...
                    if (tracks.isNullOrEmpty()) {
                        withContext(Dispatchers.Main) { android.widget.Toast.makeText(requireContext(), "Legendas: nenhuma trilha encontrada", android.widget.Toast.LENGTH_LONG).show() }
                        return@launch
                    }
                ...
                if (cues.isEmpty()) {
                    withContext(Dispatchers.Main) { android.widget.Toast.makeText(requireContext(), "Legendas: arquivo sem falas", android.widget.Toast.LENGTH_LONG).show() }
                    return@launch
                }
                withContext(Dispatchers.Main) { android.widget.Toast.makeText(requireContext(), "Legendas: " + cues.size + " falas carregadas", android.widget.Toast.LENGTH_LONG).show() }
                val overlay = TextView(requireContext()).apply {
                    setTextColor(0xFFFFFFFF.toInt())
                    textSize = 16f
                    setShadowLayer(4f, 2f, 2f, 0xFF000000.toInt())
